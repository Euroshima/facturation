...
from core.db import search_invoices, get_invoice_with_items, delete_invoice
...
class TabSearch(ttk.Frame):
    ...
    def _build_ui(self):
        ...
        e_search = ttk.Entry(st, textvariable=self.var_search, width=50)
        ...
        ttk.Button(st, text="Supprimer", command=self._delete_selected).pack(side="left", padx=6)
    ...
    def _rebuild_pdf_selected(self):
        ...
        inv_id = self.tree.item(sel[0], "values")[0]
        pdf_path, inv, _ = regenerate_pdf(inv_id)
        if not pdf_path:
    ...
    def _delete_selected(self):
        sel = self.tree.selection()
        if not sel:
            return messagebox.showinfo("Suppression", "Sélectionne une facture.")
        num = self.tree.item(sel[0], "values")[1]
        inv_id = self.tree.item(sel[0], "values")[0]
        if not messagebox.askyesno(
            "Supprimer la facture",
            f"Supprimer définitivement la facture {num} et ses lignes ?\n\n"
            "Le PDF déjà généré reste sur le disque.",
            icon="warning",
        ):
            return
        try:
            ...
            return messagebox.showerror("Suppression", f"Échec :\n{e}")
        self._do_search()
    ...
    def _email_selected(self):
        ...
        ctx = invoice_email_context(inv, client)
